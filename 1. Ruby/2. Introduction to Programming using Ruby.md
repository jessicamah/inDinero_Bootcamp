...
Form transformation involves transforming date from one form to another, like from hash to array or parsing strings as hash. This can also be rendering hashes and arrays as pdf files or web pages. Knowledge of the different forms of data or the way they are stored and represented in different data structures and output formats are needed to for data transformation. Iterators like `each` and `map` are useful tools for writing concise code and simplify these transformations.
